<template>
  <div class="sales-list">
    <!--    <button style="font-size: 30px" @click="config = {}">点击</button>-->
    <LjjBaseScrollList
        :config="config"
    />
  </div>
...
export default {
  ...
  setup(props) {
    ...
    for (let i = 0; i < salesListMockData.length; i++) {
      data[i] = []
      if (i % 2 === 0) {
        headerIndexData[i] = `
...
`
      ...
      for (let j = 0; j < headerData.length; j++) {
        let text = ''
        if (j === 0) {
          text = salesListMockData[i].order
        } else if (j === 1) {
          text = salesListMockData[i].shop
        } else if (j === 2) {
          text = salesListMockData[i].rider
        } else if (j === 3) {
          text = salesListMockData[i].newShop
        } else if (j === 4) {
          text = salesListMockData[i].avgOrder
        }
        if (j % 2 === 0) {
